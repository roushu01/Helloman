...
export default function Profile({ user, onLogout }) {
  ...
  const fetchBanks = async () => {
    try {
      ...
      const res = await api.get("/api/user-banks/all", {
      ...
      console.log("Fetch Bank Error:", err.response?.data || err.message);
    } finally {
  ...
  const saveBank = async () => {
    try {
      ...
      const res = await api.post("/api/user-banks", bankForm, {
        headers: {
      ...
      console.log("Bank Error:", err.response?.data);
    }
  };
  ...
  const deleteBank = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await api.delete(`/api/user-banks/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert("Bank deleted");
      fetchBanks();
    } catch (err) {
      console.log("Delete Bank Error:", err.response?.data || err.message);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 bg-white rounded-xl shadow-md p-5">
          <div className="flex lg:block items-center gap-4 lg:text-center border-b pb-5">
            <div className="w-16 h-16 lg:w-20 lg:h-20 lg:mx-auto rounded-full bg-orange-500 text-white flex items-center justify-center text-2xl lg:text-3xl font-bold flex-shrink-0">
              {user?.firstName?.charAt(0)}
            </div>
            <h2 className="mt-0 lg:mt-3 text-xl font-bold">
          ...
          <div className="mt-6 flex lg:flex-col gap-2 overflow-x-auto pb-2">
            ...
            <button
              onClick={onLogout}
              className="min-w-max lg:w-full flex items-center gap-2 px-4 py-3 rounded-lg whitespace-nowrap hover:bg-gray-100"
            >
              <LogOut size={20} />
              Logout
            </button>
              ...
              {orderLoading ? (
                ...
                <div className="space-y-6">
                  {orders.map((order) => (
                    <div
                      key={order._id}
                      className="border rounded-xl shadow-sm p-4 sm:p-5 bg-white"
                    >
                      {/* Order Header */}
                            ...
                            >
                              ...
                              <div className="flex flex-wrap gap-2 sm:flex-col sm:items-end">
                                <button
                                  ...
                                  className="flex-1 sm:flex-none bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg text-sm whitespace-nowrap"
                                >
                                  Write Review
                                </button>
                                ...
                                {order.orderStatus !== "Cancelled" &&
                                  order.orderStatus !== "Delivered" && (
                                    <button
                                      ...
                                      className="flex-1 sm:flex-none bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm whitespace-nowrap"
                                    >
                                      Cancel Order
                                    </button>
                                  )}
          ...
          {activeTab === "bank" && (
            <div>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
                <h2 className="text-2xl font-bold">My Bank Details</h2>
                <button
                  onClick={() => setShowBankForm(true)}
                  className="bg-orange-500 text-white px-5 py-2 rounded-lg w-full sm:w-auto"
                >
                  Add Bank
                </button>
              </div>
              ...
              {banks.filter(Boolean).map((bank) => (
                <div key={bank._id} className="border rounded-xl p-4 sm:p-5 mb-5">
                  ...
                  <p className="break-all">IFSC : {bank.ifscCode}</p>
      ...
      {showReviewModal && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-5">Write Review</h2>
            ...
            {Object.keys(bankForm)
              ...
              .map((field) => (
                <input
                  ...
                  onChange={(e) =>
                    setBankForm({ ...bankForm, [field]: e.target.value })
                  }
                />
              ))}
